...
from typing import Generic, TypeVar, Sequence, Union, Optional, Mapping
...
class Resolver(ABC):
    @abstractmethod
    @classmethod
    def raw_content(cls, node: Union[bytes, PBNode]) -> Sequence[bytes]:
        pass
    ...
    def resolve(cls,
                ...
                to_resolve: Sequence[str],
                depth: int,
                block_from_encoded_cid: Mapping[str, bytes]) -> ResolveResult:
        pass

class RawResolver(Resolver):
    @classmethod
    def resolve_content(cls, node: bytes) -> Sequence[bytes]:
        assert isinstance(node, bytes)
        return [node]

    @classmethod
    def resolve(cls, cid: CID, name: str, path: str, to_resolve: Sequence[str], depth: int, block_from_encoded_cid: Mapping[str, bytes]) -> ResolveResult:
        if len(to_resolve) > 0:
        ...
        return ResolveResult(
            entry=RawNode(
                ...
                depth=depth,
                size=len(block),
                content=cls.resolve_content(block)
            ),
            next=None
        )
    
class IdentityResolver(RawResolver):
    @classmethod
    def resolve(cls, cid: CID, name: str, path: str, to_resolve: Sequence[str], depth: int, block_from_encoded_cid: Mapping[str, bytes]) -> ResolveResult:
        if len(to_resolve) > 0:
...
class DAG_CBORResolver(Resolver):
    @classmethod
    def raw_content(cls, node: bytes | PBNode) -> Sequence[object]:
